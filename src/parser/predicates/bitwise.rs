...
impl BitwisePred {
    const BITWISE_PRED_MAP: LazyLock<HashMap<&'static str, BitwisePredType>> =
        LazyLock::new(|| HashMap::from([("-band", band as _), ("-bor", bor as _), ("-bxor", bxor as _), ("-shl", shl as _), ("-shr", shr as _)]));
...
mod tests {
    ...
    fn test_bor() {
        ...
        assert_eq!(p.safe_eval(r#" 5 -bor 5 -band 4 "#).unwrap(),"4".to_string());
        assert_eq!(p.safe_eval(r#" 5 -band 5 -bor 4 "#).unwrap(),"5".to_string());
        assert_eq!(p.safe_eval(r#" 6 -bor 5 -bor 4 "#).unwrap(),"7".to_string());
    }
    ...
    fn test_shl() {
        ...
        assert_eq!(p.safe_eval(r#" "5.5" -shl 3.5 "#).unwrap(),"96".to_string());
        assert_eq!(p.safe_eval(r#" "+5.5" -shl 3.5 "#).unwrap(),"96".to_string());
        assert_eq!(p.safe_eval(r#" "5.5as" -shl 3.5 "#).unwrap(),"".to_string());
    }
    ...
    fn test_shr() {
        ...
        assert_eq!(p.safe_eval(r#" "96.5" -shr 3.5 "#).unwrap(),"6".to_string());
        assert_eq!(p.safe_eval(r#" "+96.5" -shr 3.5 "#).unwrap(),"6".to_string());
        assert_eq!(p.safe_eval(r#" "96.5as" -shr 3.5 "#).unwrap(),"".to_string());
    }
    ...
    fn test_bnot() {
        ...
        assert_eq!(p.safe_eval(r#" [float]"+96.51e1" "#).unwrap(),"965.1".to_string());
        assert_eq!(p.safe_eval(r#" [int]"+96.51e1" "#).unwrap(),"965".to_string());
        assert_eq!(p.safe_eval(r#" [int]"+96e1" "#).unwrap(),"960".to_string());
        assert_eq!(p.safe_eval(r#" [int]"0x96" "#).unwrap(),"150".to_string());
        assert_eq!(p.safe_eval(r#" [int]"0x96e1" "#).unwrap(),"38625".to_string());
        assert_eq!(p.safe_eval(r#" [int]"+0x96e1" "#).unwrap(),"".to_string());
        assert_eq!(p.safe_eval(r#" -bnot "+96.51e1" "#).unwrap(),"-966".to_string());
    }
}
